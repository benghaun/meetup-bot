    ...
    def attendance(self):
        ...
        for i in range(len(self.dates)):
            message += self.dates[i]+' ({}):\n'.format(
                len(self.dict[self.dates[i]].split('\n')) - 1)
...
def update_user_dictionary(new_dict):
    ...
    cur.execute("""UPDATE EVENTINFO SET USERDICTIONARY ='%s'""" %
                (str(user_dictionary)))
    conn.commit()
    conn.close()
...
def get_event():
    conn, cur = connect_db()
    cur.execute(
        "SELECT GOING,NOTGOING,MAYBE,UNRESPONDED,EVENT,USERDICTIONARY FROM EVENTINFO")
    row = cur.fetchall()[0]
    conn.close()
    return Event(row)
...
def add_multidate_event(event):
    for i in range(len(event.dates)):
        cur.execute("INSERT INTO MULTIDATE (ID,DATE,GOING,EVENT) VALUES('%d','%s', '', '%s')" % (
            i, event.dates[i], event.info))

    cur.execute("INSERT INTO MULTIDATE(ID,DATE,GOING,EVENT) VALUES('%d','None','','%s')" % (
        len(event.dates)+1, event.info))
    cur.execute("INSERT INTO MULTIDATE(ID,DATE,GOING,EVENT) VALUES('%d','Unresponded', '%s', '%s')" % (
        len(event.dates)+2, event.dict['Unresponded'], event.info))
    conn.commit()
    conn.close()
...
def update_multidate_event(event):
    ...
    for date in event.dates:
        cur.execute("UPDATE MULTIDATE SET GOING = '%s' WHERE DATE = '%s'" % (
...
custom_keyboard_with_cancel = [
    [going_button, notgoing_button], [maybe_button], [cancel_button]]
reply_markup_With_cancel = telegram.InlineKeyboardMarkup(
    custom_keyboard_with_cancel)
...
def create_event_multidate(bot, update):
    ...
    for date in multidate_event.dates:
        if len(subkeyboard) < 3:
            subkeyboard.append(telegram.InlineKeyboardButton(
                text=date, callback_data='date:'+date))
        else:
            keyboard.append(subkeyboard)
            subkeyboard = [telegram.InlineKeyboardButton(
                text=date, callback_data='date:'+date)]
    ...
    keyboard.append(
        [telegram.InlineKeyboardButton(text='OK', callback_data='OK')])
    ...
    for user in user_dictionary:
        ...
        bot.sendMessage(chat_id=user_dictionary[user],
                        ...
                        reply_markup=reply_markup)
        print "done sending to user %user"
    add_multidate_event(multidate_event)


dispatcher.add_handler(CommandHandler(
    'multidate_event', create_event_multidate))
...
def single_event_response(bot, update):
    ...
    if query.data == 'single event:going':
        ...
        event.unresponded = event.unresponded.replace(
            '%s\n' % query.from_user.first_name, '')
...
dispatcher.add_handler(CallbackQueryHandler(
    single_event_response, pattern="^single event:"))
...
def multidate_event_response(bot, update):
    ...
    if query.data[:5] == "date:":  # update database with dates that people can go
        ...
        for i in range(len(multidate_event.dates)-2):
            ...
            if (name not in multidate_event.dict[current_date]):
                if len(subkeyboard) < 3:
                    subkeyboard.append(telegram.InlineKeyboardButton(
                        text=current_date, callback_data='date:' + current_date))
                else:
                    keyboard.append(subkeyboard)
                    subkeyboard = []
                    subkeyboard.append(telegram.InlineKeyboardButton(
    ...
    if query.data == 'OK':  # all dates selected, send message to group
        ...
        multidate_event.dict["Unresponded"] = multidate_event.dict["Unresponded"].replace(
            '%s\n' % name, '')
...
dispatcher.add_handler(CallbackQueryHandler(
    multidate_event_response, pattern="^date:|OK"))
...
def change(bot, update):
    ...
    # multidate event
        ...
        if update.message.chat.type == 'private':
            ...
            for date in multidate_event.dict:
                multidate_event.dict[date] = multidate_event.dict[date].replace(
                    update.message.from_user.first_name + "\n", "")
            ...
            for i in range(len(dates)-2):
                if len(subkeyboard) < 3:
                    subkeyboard.append(telegram.InlineKeyboardButton(
                        text=dates[i], callback_data='date:'+dates[i]))
                else:
                    keyboard.append(subkeyboard)
                    subkeyboard = []
                    subkeyboard.append(telegram.InlineKeyboardButton(
